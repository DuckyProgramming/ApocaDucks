class projectile{
    constructor(layer,x,y,type,direction,id,damage,time,crit,index,args=[]){
		...
		this.exploder=this.type==2||this.type==16||this.type==21||this.type==22||this.type==26||
			...
			this.type==206||this.type==211||this.type==213
		switch(this.type){
			...
			case 209: case 210: case 211: case 216: case 220:
				...
				this.velocity={x:this.speed*lsin(this.direction),y:this.speed*lcos(this.direction)-(this.type==68||this.type==132||this.type==135||this.type==136||this.type==169||this.type==170||this.type==206||this.type==210?0:4)}
				this.goal=-1
				if(this.type==135||this.type==136||this.type==169||this.type==170){
					...
					this.type==182||this.type==183||this.type==210
				){
					...
					this.type==209||this.type==216||this.type==220
				){
					this.stop=false
					if(this.type==117){
						this.ammo=10
			...
			case 213:
				for(let b=0,lb=entities.players.length;b<lb;b++){
					...
					if((entities.players[b].id!=this.id&&game.pvp||entities.players[b].id==0&&this.id!=0||entities.players[b].id!=0&&this.id==0)&&entities.players[b].explodable()&&entities.players[b].life>0&&dist(entities.players[b].position.x,entities.players[b].position.y,this.position.x,this.position.y)<600){
				        entities.projectiles.push(new projectile(this.layer,this.position.x,this.position.y,1,atan2(entities.players[b].position.x-this.position.x,this.position.y-entities.players[b].position.y),this.id,this.damage/3,300,this.crit,this.index))
                    }
				}
			break
		}
    ...
    update(){
        ...
        for(let a=0,la=(this.type==125||this.type==126||this.type==127||this.type==130||this.type==173||this.type==174||this.type==185||this.type==192||this.type==196||this.type==197||this.type==198||this.type==199)?2:(this.type==4||this.type==14||this.type==39||this.type==50||this.type==57||this.type==88||this.type==94||this.type==167||this.type==175||this.type==186||this.type==203)?6:4;a<la;a++){
			switch(this.type){
				...
				break
				case 120: case 177: case 178: case 179: case 180: case 182: case 183: case 210:
					if(a==2){
						this.midpoint.position.x=this.position.x
						this.midpoint.position.y=this.position.y
					}
			...
			if(this.active&&this.type!=85&&this.type!=190&&this.type!=191){
				for(let b=0,lb=entities.players.length;b<lb;b++){
				    if(inBoxBox(this,entities.players[b])&&(((this.id==0?1:0)!=(entities.players[b].id==0?1:0)||this.id==-1||game.pvp&&this.id!=entities.players[b].id)||(this.type==9||this.type==10||this.type==11||this.type==38||this.type==63||this.type==72||this.type==82||this.type==194||this.type==216&&entities.players[b].life<entities.players[b].base.life*2)&&!entities.players[b].playerData.name.includes('Medic'))&&
						...
						!((this.type==113||this.type==114||this.type==115||this.type==116||this.type==117||this.type==146||this.type==181||this.type==209||this.type==216||this.type==220)&&this.timer<150)&&
						...
						if(entities.players[b].weaponType!=370&&entities.players[b].weaponType!=381){
							if(this.type==12){
								...
								entities.players[b].shrinkTime=max(entities.players[b].shrinkTime,30)
							}
							if(this.type==20){
