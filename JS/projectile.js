class projectile{
    constructor(layer,x,y,type,direction,id,damage,time,crit,index){
		...
		switch(this.type){
			...
			break
			case 2: case 16: case 21: case 22:
				this.speed=5
				this.time=time
			break
			case 3:
    ...
    update(){
		...
		if(
			...
			this.type==19||this.type==20||this.type==23||this.type==24
		){
			this.fade=smoothAnim(this.fade,this.active,0,1,5)
		}else if(this.type==2||this.type==3||this.type==16||this.type==21||this.type==22){
            ...
            if(this.active){
                for(let a=0,la=entities.players.length;a<la;a++){
                    if(inBoxBox(this,entities.players[a])&&((this.id==0?1:0)!=(entities.players[a].id==0?1:0)||this.type==9||this.type==10||this.type==11)&&entities.players[a].life>0&&entities.players[a].invincible<=0&&this.active){
						...
						if(this.type==2||this.type==16||this.type==21||this.type==22){
							this.explode()
						}
                    }
                }
