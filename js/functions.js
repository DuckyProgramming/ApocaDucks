...
function displayMain(layer,effective,keyStore){
    ...
    if(
        ...
        menu.level==41||menu.level==42||menu.level==43||menu.level==44
    ){
        image(
            graphics.overlay[0],
...
function generateLevel(info,layer){
    ...
    switch(game.level){
        case 29: case 36: case 37: case 41: case 43:
            game.placer=[[],[],[],[]]
        break
    }
    ...
    if((game.level==36||game.level==41)&&entities.walls.length>0){
        for(let a=0,la=entities.walls[1].length;a<la;a++){
    ...
    let reject=[]
    for(let g=0,lg=game.level==41?0:game.level==25||game.level==26||game.level==37||game.level==42?2:1;g<lg;g++){
                    ...
                    switch(level[a][b]){
                        case '>':
                            if(!reject.includes(a*lb+b)){
                                ...
                                }else if((game.level==25||game.level==26||game.level==32||game.level==33||game.level==37||game.level==38||game.level==40||game.level==41||game.level==42||game.level==43)&&(
                                    level[a][b-1]=='.'||
                    ...
                    switch(level[a][b]){
                        case '[':
                            if(!reject.includes(a*lb+b)){
                                ...
                                }else if((game.level==25||game.level==26||game.level==37||game.level==38||game.level==40||game.level==41||game.level==42||game.level==43)&&(
                                    level[a][b-1]=='.'||
                                    level[a][b-1]=='c'||
                                    level[a][b-1]=='%'&&level[a][b-2]=='%'&&level[a][b-3]=='%'&&game.level==40
                                )){
        ...
        for(let b=0,lb=level[a].length;b<lb;b++){
            switch(level[a][b]){
				case '#': case '.':
                    ...
                    }else if(level[a][b]=='.'&&(game.level==25||game.level==26||game.level==32||game.level==33||game.level==37||game.level==38||game.level==40||game.level==41||game.level==42||game.level==43)&&!reject.includes(a*lb+b)){
                        let extent=0
                ...
                break
                case '!':
                    let cluster=game.peakWeapon?1:game.level==36||game.level==41?0:game.level==27&&game.pvp?1:floor(random(1.5))
                    switch(game.level){
                        case 27:
                            if(game.pvp){
                    ...
                    switch(game.level){
                        ...
                        default:
                            if(!reject.includes(a*lb+b)){
                                ...
                                }else if(game.level==37||game.level==38||game.level==41){
                                    if(level[a][b-1]=='>'&&game.level==22){
                                        entities.walls[1].push(new wall(graphics.main,game.tileset[0]/2+(b+extent/2-0.5)*game.tileset[0],(a+0.15)*game.tileset[1],game.tileset[0]*(2+extent),game.tileset[1]*0.3,37))
                ...
                case '~':
                    switch(game.level){
                        ...
                        case 38: case 40: case 41: case 42:
                            entities.walls[1].push(new wall(graphics.main,game.tileset[0]/2+b*game.tileset[0],game.tileset[1]/2+a*game.tileset[1],game.tileset[1]*0.6,game.tileset[1]*0.6,16))
                            let cluster=game.peakWeapon?1:game.level==38||game.level==41?0:game.level==27&&game.pvp?0:floor(random(1.5))
                            entities.walls[1][entities.walls[1].length-1].weapon=listing[cluster][floor(random(listing[cluster].length))]
                        break
                        case 28:
                            entities.walls[1].push(new wall(graphics.main,game.tileset[0]/2+b*game.tileset[0],game.tileset[1]/2+a*game.tileset[1],game.tileset[1]*0.6,game.tileset[1]*0.6,16))
                ...
                case ':':
                    switch(game.level){
                        ...
                        break
                        default:
                            if(game.level==22||a>10&&a<la-10&&(game.level==23||game.level==35)||game.level==37||game.level==43){
    ...
    }else if(game.level==40||game.level==43){
        graphics.gradient=[new p5.LinearGradient(85,graphics.main[0].height)]
        graphics.gradient[0].colors(
            0.0,color(-75,-25,-40),
            1.0,color(30,40,60)
    ...
    for(let c=0,lc=game.players;c<lc;c++){
        let clump=listing[game.peakWeapon?1:game.level==27&&game.pvp||game.level==37||game.level==38?0:floor(random(0,1.5))]
        let type=game.level==36||game.level==41?0:game.selector?findName('PlayerSelector',types.player):game.randomizer?floor(random(listing[1][listing[1].length-1]+1,types.player.length)):game.classicWeapon||c>=game.gaming?(game.past?weapon:clump[floor(random(0,clump.length))]):(game.level==13||game.level==14?0:game.weapon[game.mainline?lc:c][game.weaponTick[c]%game.weapon[game.mainline?lc:c].length])
        for(let a=0,la=level.length;a<la;a++){
            for(let b=0,lb=level[a].length;b<lb;b++){
                if(game.attacker&&game.level!=13&&game.level!=14){
                    ...
                    if(int(level[a][b])==(game.level==34?0:game.level==32?1:game.level==27?c%5:c)+1&&(!game.pvp||game.level==13||game.level==14)){
                        entities.players.push(new player(layer,game.tileset[0]/2+b*game.tileset[0],game.tileset[1]/2+a*game.tileset[1],c+1,0,[],true,type,game.index))
                        if(!game.classicWeapon&&c<game.gaming&&game.level!=13&&game.level!=14){
                            game.weaponTick[c]++
                        }
    ...
    for(let c=0,lc=game.players;c<lc;c++){
        ...
        if(game.level==36||game.level==41){
    ...
    if(display.cycle>=types.mission[game.mission].wave.length){
        ...
        for(let a=0,la=types.mission[game.mission].wave[display.cycle].length;a<la;a++){
            ...
            if(types.mission[game.mission].wave[display.cycle][a][0].includes('Boss')){
                ...
                for(let b=0,lb=ceil(types.mission[game.mission].wave[display.cycle][a][1]*(game.level==37?game.players*0.1+0.25:game.level==29?game.players*0.1+0.4:game.players*0.25+0.25)*(game.classicRespawn?1.25:1)*(game.level==7?0.6:1)*(game.level==8?(game.attacker?0.75:1.5):1)*(game.level==16?0.1:1)*(game.level==17?(game.attacker?0.4:1):1)*(game.level==19||game.level==31||game.level==42?2.5:1)*(game.level==29||game.level==37?(types.mission[game.mission].wave.length==1?0.3:1.8):1)*(game.level==32||game.level==33?1.2:1)*(game.level==42?0.75:1)*(game.peakWeapon?2:1)*game.diff);b<lb;b++){
                    game.stack.push([spy?-1:floor(random(0,6))+6,types.mission[game.mission].wave[display.cycle][a][0]])
                }
            }
        }
    ...
    if(game.newWave){
                        ...
                        for(let b=0,lb=levels[game.level][a].length;b<lb;b++){
                            for(let c=0,lc=game.players;c<lc;c++){
                                if(levels[game.level][a][b]==['67890','12345','ABCDE'][display.cycle][(c+shifter)%5]){
                                    ...
                                    entities.players[c].base.position.y=(a+0.5)*game.tileset[0]
                                    entities.players[c].respawn()
                                }
                            }
                        }
        ...
        if(game.level==8){
            ...
            if(game.respawners[0]>=game.players/8&&game.point[0]==1){
                for(let a=0,la=entities.players.length;a<la;a++){
                    if(entities.players[a].id==1&&(entities.players[a].die.timer>360||entities.players[a].die.timer>240&&game.point[1]==1||entities.players[a].die.timer>120&&game.point[3]==1)&&entities.players[a].life<=0){
                        entities.players[a].respawn()
                    }
                }
            }
            if(game.respawners[1]>=game.players/8&&game.point[4]==2){
                for(let a=0,la=entities.players.length;a<la;a++){
                    if(entities.players[a].id==2&&(entities.players[a].die.timer>360||entities.players[a].die.timer>240&&game.point[3]==2||entities.players[a].die.timer>120&&game.point[1]==2)&&entities.players[a].life<=0){
                        entities.players[a].respawn()
                    }
                }
            }
            ...
            if(game.level==29){
                ...
                if((total<(game.level==23?((1+game.players)*(game.peakWeapon?7.5:5)):(game.attacker?1:4))&&(subTotal==0||game.level==23)||game.level==33)&&!(display.cycle==types.mission[game.mission].wave.length&&total>0)){
                    ...
                    if(display.wait<=0){
                        ...
                        if(!game.perpetual&&!game.nuke&&!(game.level==27&&game.pvp)&&!(game.level==28&&game.pvp)&&!(game.level==36&&game.pvp)&&!(game.level==38&&game.pvp)&&!(game.level==41&&game.pvp)&&!(game.level==43&&game.pvp)&&!(game.level==44&&game.pvp)){
                            newWave()
                        }
                    }
                }
