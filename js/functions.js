...
function generateLevel(level,layer){
                        ...
                        if(!reject.includes(a*lb+b)){
                            ...
                            for(let e=1,le=min(a+1,level[0].length-b);e<le;e++){
                                if(level[a-e][b+e]=='['){
                                    let valid=true
                                    for(let f=0,lf=e;f<lf;f++){
                                        if(level[a-e][b+f]!='#'||reject.includes((a-e)*lb+(b+f))){
                                            valid=false
                                        }
                                    }
                                    if(valid){
            ...
            switch(level[a][b]){
                ...
                case '_':
                    switch(game.level){
                        ...
                        case 23:
                            if(level[a][b-1]=='['){
                                entities.walls[0].push(new wall(graphics.main,b*game.tileset[0],(a+0.15)*game.tileset[1],game.tileset[0]*2,game.tileset[1]*0.3,24))
                            }else if(level[a][b+1]==']'){
                                entities.walls[0].push(new wall(graphics.main,game.tileset[0]+b*game.tileset[0],(a+0.15)*game.tileset[1],game.tileset[0]*2,game.tileset[1]*0.3,24))
                            }else{
                ...
                case 'c':
                    if(game.level==21){
                        ...
                        entities.walls[0].push(new wall(graphics.main,game.tileset[0]/2+b*game.tileset[0],(a+0.5)*game.tileset[1],game.tileset[0],game.tileset[1],35))
                    }else{
                ...
                case 'd':
                    ...
                    }else if(game.level==19||game.level==22||game.level==23){
                        entities.walls[0].push(new wall(graphics.main,game.tileset[0]/2+b*game.tileset[0],game.tileset[1]/2+(a-0.5)*game.tileset[1],game.tileset[0],game.tileset[1]*2,17))
                    }else{
    ...
    if(game.level==22||game.level==23){
        for(let a=0,la=entities.walls[0].length;a<la;a++){
            if(entities.walls[0][0].type==17||entities.walls[0][0].type==18||entities.walls[0][0].type==20||entities.walls[0][0].type==21){
                entities.walls[0].splice(entities.walls[0].length-1,0,entities.walls[0][0])
                entities.walls[0].splice(0,1)
            }else{
                a=la
            }
        }
        let set=[
            ...
            [99,35.5,[-90,-90,90,90,-90],[0,1,2,3,4]],
...
function checkEnd(level,layer,key){
    if(game.past){
        ...
        if(game.stack.length>0&&!game.assault&&!game.nuke){
            ...
            if(total<(game.level==23?(1+game.players)*(game.peakWeapon?9:6):(game.attacker?1:4))&&(subTotal==0||game.level==23)&&!(display.cycle==types.mission[game.mission].wave.length&&total>0)){
                display.wait--
                if(display.wait<=0){
                    display.wait=game.level==23?900:240
...
function outWalls(){
    ...
    for(let a=0,la=entities.walls.length;a<la;a++){
        for(let b=0,lb=entities.walls[a].length;b<lb;b++){
            totals[entities.walls[a][b].type]++
        }
    }
    print(totals)
}
