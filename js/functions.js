...
function displayMain(layer,effective,keyStore){
    ...
    if(game.level==19||game.level==22||game.level==23||game.level==24||game.level==25||game.level==26||game.level==27||game.level==28){
        image(
            graphics.overlay[0],
                    ...
                    switch(level[a][b]){
                        case '>':
                            if(!reject.includes(a*lb+b)){
                                ...
                                reject.push(a*lb+b)
                                if((game.level==25||game.level==26)&&level[a][b-1]=='m'){
        ...
        for(let a=0,la=level.length;a<la;a++){
            for(let b=0,lb=level[a].length;b<lb;b++){
                ...
                if(game.attacker&&game.level!=13&&game.level!=14){
                    ...
                    let encode=(game.level==23||game.level==26||game.level==28)&&c<4?'qwer'[(c+shifter)%4]:'qwerty'[c]
        ...
        case 27:
            if(game.pvp){
                ...
                for(let a=0,la=position.length;a<la;a++){
                    ...
                    switch(position[a]){
                        case 1:
                            entities.players[a].position.x=location[ticker[0]].x
                            entities.players[a].position.y=location[ticker[0]].y
                            ticker[0]++
                        break
                        case 2:
                            entities.players[a].position.x=location[ticker[1]+2].x
                            entities.players[a].position.y=location[ticker[1]+2].y
                            ticker[1]++
                        break
                        case 3:
        ...
        if(game.stack.length>0&&!game.assault&&!game.nuke){
            ...
            if(total<(game.level==23?((1+game.players)*(game.peakWeapon?7.5:5)):(game.attacker?1:4))&&(subTotal==0||game.level==23)&&!(display.cycle==types.mission[game.mission].wave.length&&total>0)){
                ...
                if(display.wait<=0){
                    ...
                    if(!game.perpetual&&!game.nuke&&!(game.level==27&&game.pvp)){
                        newWave(level,layer)
                    }
                }
            }
...
function initialGraphics(){
    ...
    if(menu.level==19||menu.level==22||menu.level==23||menu.level==24||menu.level==25||menu.level==26||menu.level==27||menu.level==28){
