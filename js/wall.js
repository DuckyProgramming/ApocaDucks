class wall{
    constructor(layer,x,y,width,height,type){
        ...
        this.standard=this.type!=3&&this.type!=5&&this.type!=7&&this.type!=8&&this.type!=9&&this.type!=10&&this.type!=11&&this.type!=12&&this.type!=14&&this.type!=16&&this.type!=27&&this.type!=31&&this.type!=33&&this.type!=36&&this.type!=39&&this.type!=42&&this.type!=55
    ...
    set(){
        ...
        switch(this.type){
            ...
            break
            case 31:
                this.owner=game.level==27&&game.pvp?[1,1,2,2][this.pos]:(game.level==22||game.level==23||game.level==35)&&!game.pvp&&!game.attacker?floor(random(1,game.players+1)):-1
            ...
            break
            case 33:
                this.owner=game.level==29||game.level==37?0:game.level==27&&game.pvp?[1,1,2,2][this.pos]:(game.level==22||game.level==23||game.level==25||game.level==26||game.level==32||game.level==35)&&!game.pvp&&!game.attacker?floor(random(1,game.players+1)):-1
            ...
            case 42:
                if(game.level==30){
                    ...
                    this.owner=game.level==32?0:(game.level==22||game.level==23||game.level==25||game.level==26||game.level==35)&&!game.pvp&&!game.attacker?floor(random(1,game.players+1)):-1
                    entities.players.push(new player(graphics.main[1],this.position.x,this.position.y-50,this.owner,0,[],false,findName(game.level==32?'Payload':'Rogue',types.player),game.index))
                    game.index++
    ...
    checkRedundancy(){
        for(let a=0,la=entities.walls.length;a<la;a++){
            for(let b=0,lb=entities.walls[a].length;b<lb;b++){
                let c=entities.walls[a][b]
                if(c.standard&&this.type!=37&&c.type!=37&&(this.type!=38&&c.type!=38||game.level!=22&&game.level!=23&&game.level!=33&&game.level!=37)&&this.type!=56&&c.type!=56&&!((c.type==20||c.type==21)&&this.type==24&&game.level==30)&&(c.position.x!=this.position.x||c.position.y!=this.position.y)){
    ...
    display(layer,offsetX=0,offsetY=0){
        ...
        switch(this.type){
            ...
            case 31: case 33: case 36:
                ...
                layer.textSize(20)
                let texts=''
                if(game.level==19||game.level==24||game.level==27||game.level==34){
                    switch(this.type){
                        case 31:
                            texts='Fort'
                        break
                    ...
                    texts=game.level==37?'Gate':game.level==36?'Point':game.level==29?'Target':game.level==28?['Turret','Point ','Turret','Turret','Guster','Guster','Guster','Guster'][this.pos]:this.pos>=3&&(game.level==25||game.level==26)||this.pos>=5?'Node':'Point '+'ABCDE'[game.level==28?0:this.pos]
                }
            ...
            case 31: case 33: case 36: case 42:
                if(game.level==30){
                    ...
                    if(game.level==19||game.level==24||game.level==27||game.level==34||game.level==36){
                        ...
                        texts=game.level==37?'Gate':game.level==29?'Target':game.level==28?['Turret','Point ','Turret','Turret','Guster','Guster','Guster','Guster'][this.pos]:this.type==42?'Payload':this.pos>=3&&(game.level==25||game.level==26)||this.pos>=5?'Node':'Point '+'ABCDE'[game.level==28?0:this.pos]
                    }
                    graphics.overlay[0].fill(255)
                    graphics.overlay[0].textSize(10)
                    ...
                    if(this.type==31&&game.level==19&&game.pvp&&this.pos==2||this.type==31&&game.level==24||this.type==33&&game.level==23&&this.pos==4||game.level==28&&this.pos==1||this.type==31&&game.level==31&&game.pvp&&this.pos==3||game.level==33&&this.pos==0){
                        for(let a=0,la=this.timers.length;a<la;a++){
                            graphics.overlay[0].fill(255)
                            graphics.overlay[0].textSize(10)
        ...
        switch(this.type){
            ...
            case 38:
                if(game.level==22){
                    ...
                    }else if((abs(this.position.x-game.tileset[0]*70.5)<1||abs(this.position.x-game.tileset[0]*126.5)<1)&&this.time%15==0){
                        for(let a=0,la=entities.players.length;a<la;a++){
                ...
                for(let b=0,lb=this.collide[a].length;b<lb;b++){
                    ...
                    if(
                        ...
                        switch(this.type){
                            ...
                            break
                            case 33:
                                if(game.level!=19&&game.level!=29&&game.level!=36&&!c.construct&&!c.sidekick&&!c.fort&&c.id>0&&!game.attacker&&c.weapon.uses<=0&&!((game.level==22||game.level==23||game.level==25||game.level==26||game.level==27||game.level==32||game.level==33)&&!(c.id==this.owner||game.level==27&&game.pvp&&(c.id==1&&this.pos==0||c.id==2&&this.pos==3||c.id==3)||this.owner>0&&c.id>0&&!game.pvp))){
                                    if(c.auto){
                                        c.newWeaponSet(c.type)
                                    }else{
                                        c.newWeapon()
