    ...
    display(layer,offsetX=0,offsetY=0){
        ...
        switch(this.type){
            ...
            case 4:
                switch(game.level){
                    ...
                    case 23:
                        layer.fill(110,105,100)
                        layer.rect(0,0,this.width+1,this.height+1)
                        layer.fill(120,200-min(480,this.reload)/5,120,120)
                        layer.quad(
                            -this.width/2,-this.height/2-0.5,
                            -this.width/2+15,-this.height/2+15,
                            this.width/2-15,-this.height/2+15,
            ...
            case 39:
                ...
                layer.fill(200)
                if((
                    entities.players[game.players+[1,4,0,2,3][this.point]].life<entities.players[game.players+[1,4,0,2,3][this.point]].base.life*0.8||
                    entities.players[game.players+[1,4,0,2,3][this.point]].id==0
                )&&!game.pvp){
                    layer.fill(255,50,50)
                }
                if(game.pvp){
                    switch(entities.players[game.players+[1,4,0,2,3][this.point]].id){
                        case 1:
                            layer.fill(15,75,255)
                        break
                        case 2:
    ...
    displayOver(layer){
        ...
        switch(this.type){
            ...
            case 31: case 33: case 36:
                ...
                if(this.type==31&&game.level==24||this.type==33&&game.level==23&&this.pos==4){
    ...
    update(){
        ...
        switch(this.type){
            ...
            case 11:
                if(game.level==22&&(abs(this.base.position.y-game.tileset[1]*49.75)<1||abs(this.base.position.y-game.tileset[1]*48.75)<1)&&!game.point[1]&&this.height>0){
                    ...
                    this.height=smoothAnim(this.height,game.point[1]>0&&game.point[1]<=game.players&&(
                            abs(entities.players[game.point[1]-1].position.x-this.position.x)<150&&abs(entities.players[game.point[1]-1].position.y-this.position.y)<60||
                            abs(entities.players[game.players+3].position.x-this.position.x)<150&&abs(entities.players[game.players+3].position.y-this.position.y)<60&&entities.players[game.players+3].id==game.point[1]
            ...
            case 16:
                ...
                if(this.recharge>0&&(game.level==15||game.level==18||game.level==19||game.level==22||game.level==23||game.level==24)){
                    this.recharge--
                }
                if(this.falling>0){
                    let speed=1
                ...
                if(game.level==23){
                    this.visible=smoothAnim(this.visible,game.point[2]>0&&game.point[2]<=game.players&&(abs(entities.players[game.point[2]-1].position.x-this.position.x)<300&&abs(entities.players[game.point[2]-1].position.y-this.position.y)<120),0,1,0.1)
                }
            break
            case 31:
            ...
            case 38:
                if(game.level==22){
                    if((
                        ...
                        abs(this.position.x-game.tileset[0]*137.5)<1&&!game.point[2]
                    )&&this.height>0){
                        this.height-=0.1
                        this.bounder.height-=0.1
                        this.position.y-=0.05
                        ...
                        switch(this.type){
                            ...
                            case 16:
                                if(game.level==16){
                                    ...
                                    if(!(game.level==22&&game.pointAnim[2]>=1)&&!(game.level==23&&this.visible<1)){
                                        c.newWeaponSet(this.weapon)
                                        let chunk=game.peakWeapon?1:floor(random(0,1.5))
                                        this.weapon=listing[chunk][floor(random(listing[chunk].length))]
                                    ...
                                    switch(d){
                                        ...
                                        case 1:
                                            ...
                                            switch(this.type){
                                                ...
                                                break
                                                case 4:
                                                    if(this.reload<=0&&(c.id>0||game.attacker||game.level==17||game.level==18||game.level==19)&&c.life>0&&c.attacking&&!(c.construct&&game.level==19)){
                                                        if(game.attacker||game.level==17||game.level==18){
                                                            this.align=c.id
                                                        }
                                                        this.reload=480
                                                ...
                                                case 32:
                                                    if(this.reload<=0&&(!c.fort||c.id==0)&&(c.id>0||game.attacker||game.level==17||game.level==18||game.level==19)&&c.life>0&&c.attacking){
                                                        ...
                                                        switch(game.level){
                                                            case 19:
                                                                for(let e=0,le=entities.walls[0].length;e<le;e++){
                                                                    if(entities.walls[0][e].type==31&&abs(this.position.x-entities.walls[0][e].position.x)<600&&(entities.walls[0][e].owner==c.id&&c.id>=0||entities.walls[0][e].owner>0&&c.id>0&&!game.pvp)){
                                                                        hit=true
                                                                        entities.players.push(new player(graphics.main[1],this.position.x,this.position.y-50,entities.walls[0][e].owner,0,[],false,floor(random(findName('ConstructMachineGun',types.player),findName('ConstructRemote',types.player))),game.index))
                                                                        game.index++
                                                                        entities.players[entities.players.length-1].constructify()
