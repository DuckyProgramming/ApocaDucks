class projectile{
    constructor(layer,x,y,type,direction,id,damage,time,crit,index,args=[]){
		...
		switch(this.type){
			...
			case 7: case 23: case 37: case 40: case 46: case 76: case 77:
				...
				this.time=5
			break
			case 15: case 74: case 75: case 222: case 223: case 287: case 288: case 371: case 449:
    ...
    update(){
        ...
        if(this.fade<=0){
			this.remove=true
			if(this.type==451&&this.hit.length==0){
				for(let a=0,la=entities.players.length;a<la;a++){
					if(entities.players[a].index==this.index&&!entities.players[a].sidekick){
        ...
        for(let a=0,la=this.stun>0?2:this.rules.slow?2:this.rules.fast?6:4;a<la;a++){
			switch(this.type){
				...
				break
				case 222: case 223: case 287: case 288: case 319: case 371: case 374: case 424: case 449:
				    this.position.x+=this.speed*lsin(this.direction)
				    this.position.y-=this.speed*lcos(this.direction)
					this.travel+=this.speed
					if(this.travel>(this.id==0?750:1500)){
