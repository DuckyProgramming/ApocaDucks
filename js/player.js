        ...
        if(this.construct&&!this.remote||this.fort&&!this.auto){
            ...
            if(floor(random(0,30))==0||abs(this.position.x-this.target.position.x)<10){
                if(game.attacker&&this.id==0&&!this.free&&this.playerData.name!='Buster'){
                    ...
                    this.manage[1]=false
                    if(!game.noPlayer&&(this.target.point==-1||floor(random(0,(this.life<=0?15:150)))==0||this.target.point>=0&&(entities.players[game.players+[1,4,0,2,3,6,5,8,7][this.target.point]].id==this.id?1:0)==(this.weaponType>=0?1:0))){
