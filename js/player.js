class player{
            ...
            if(game.newStats){
                if(this.playerData.name=='PlayerSpy'){
                    ...
                    if(game.level==13){
                        ...
                        layer.text(`Lead Time: ${formatTime(this.stats.usurp)}\nDeaths: ${this.stats.deaths}\nWeapon: ${this.weaponType==-1?`None`:this.weaponData.name+(this.playerData.name==`PlayerConglomeration`||this.playerData.name==`PlayerSelector`?`[${this.subWeaponAData.name},${this.subWeaponBData.name}]`:this.playerData.name==`PlayerSwitcheroo`||this.playerData.name==`PlayerSwapper`?`[${this.subWeaponAData.name}]`:``)}`,0,-35)
    ...
    newWeapon(){
        if(game.selector&&this.id>0&&this.id<=game.gaming){
            ...
            let clump=listing[game.peakWeapon?1:game.level==36?0:(game.level==27||game.level==38)&&game.pvp?0:floor(random(0,1.5))]
            this.type=clump[floor(random(0,clump.length))]
        }else if(this.id<=game.weapon.length){
            if((game.level==27||game.level==38)&&game.pvp){
    ...
    initialWeapon(){
        ...
        for(let a=0,la=entities.projectiles.length;a<la;a++){
            if(entities.projectiles[a].index==this.index&&!entities.projectiles[a].trap){
                entities.projectiles[a].time=min(300,entities.projectiles[a].time)
            }
        }
        switch(this.playerData.name){
            ...
            break
            case 'PlayerSelector':
                let tick=game.peakWeapon?1:(game.level==27||game.level==38)&&game.pvp?0:floor(random(0,1.5))
    ...
    unProtected(){
        ...
        if(this.fort&&game.pvp&&game.level==42){
            let local=0
            for(let a=0,la=entities.players.length;a<la;a++){
                if(entities.players[a].fort&&abs(entities.players[a].pos-this.pos)==1&&entities.players[a].id==this.id&&this.id>0&&this.pos!=7&&entities.players[a].pos!=7){
                    local++
                }
            }
            if(local==2&&this.pos!=4){
    ...
    takeDamage(damage){
        ...
        if(
            ...
            this.life-=damage*(game.level==27&&(game.pvp?this.index+1:this.id)>game.gaming&&(!game.pvp||game.gaming==1)?0.8:1)*(this.vulnerableTime>0?3:1)*(this.defendBuff>0?0.5:1)*(
                ...
                this.playerData.name=='SidekickGuillotine'||this.playerData.name=='PlayerIrony'
                ?2/3:
	...
	attack(variant){
        ...
        if(valid){
            ...
            let bypass=false
            switch(weaponType){
                case 0: case 177: case 257: case 578: case 582:
                    for(let a=0,la=10;a<la;a++){
                ...
                case 620:
                    ...
                    for(let a=0,la=entities.players.length;a<la;a++){
                        if((entities.players[a].id!=this.id&&game.pvp||entities.players[a].id==0&&this.id!=0||entities.players[a].id!=0&&this.id==0||entities.players[a].id==-1||this.id==-1)&&entities.players[a].life>0&&dist(entities.players[a].position.x,entities.players[a].position.y,this.position.x,this.position.y)<900){
                            let distance=dist(entities.players[a].position.x,entities.players[a].position.y,this.position.x,this.position.y)
                            if(abs(this.position.y-entities.players[a].position.y)<abs(this.position.x-entities.players[a].position.x)*0.25){
                                if(entities.players[a].position.x<this.position.x){
                ...
                break
                case 638:
                    entities.projectiles.push(new projectile(this.layer,spawn[0],spawn[1],321,(lsin(this.direction.main)<0?-90:90)+random(-15,15),this.id,weaponData.damage*damageBuff,10,crit,this.index))
                break
                case 639:
                ...
                case 649:
                    for(let a=0,la=10;a<la;a++){
                        entities.projectiles.push(new projectile(this.layer,spawn[0],spawn[1],325,(lsin(this.direction.main)<0?-90:90)+random(-20,20),this.id,weaponData.damage*damageBuff,15,crit,this.index))
                    }
                break
                case 650:
                ...
                case 667:
                    ...
                    for(let a=0,la=entities.players.length;a<la;a++){
                        if((entities.players[a].id!=this.id&&game.pvp||entities.players[a].id==0&&this.id!=0||entities.players[a].id!=0&&this.id==0||entities.players[a].id==-1||this.id==-1)&&entities.players[a].life>0){
                            ...
                            if(!fired667[0]&&distance==minimum667[0]){
                                for(let b=0,lb=10;b<lb;b++){
                                    entities.projectiles.push(new projectile(this.layer,this.position.x,this.position.y,1,atan2(entities.players[a].position.x-this.position.x,this.position.y-entities.players[a].position.y)+random(-20,20),this.id,this.weaponData.damage*this.playerData.damageBuff,300,crit,this.index))
                                }
                                fired667[0]=true
                            }
                    ...
                    if(!fired667[0]){
                        for(let a=0,la=10;a<la;a++){
                            entities.projectiles.push(new projectile(this.layer,this.position.x,this.position.y,1,-90+random(-20,20),this.id,this.weaponData.damage*this.playerData.damageBuff,300,crit,this.index))
                        }
                    }
                    if(!fired667[1]){
                        for(let a=0,la=10;a<la;a++){
    ...
    fortHealth(){
        ...
        if(this.id>0&&game.level!=29&&game.level!=38){
            this.multLife(game.level==23||game.level==24||game.level==26?1.5:game.level==40?3:2)
        }
        if(game.peakWeapon){
            this.multLife(1.5)
        }
    ...
    validTarget(target){
        return (this.id==0&&target.id!=0||this.id!=0&&target.id==0||game.pvp&&this.id!=target.id||this.id==-1&&target.id!=-1||target.id==-1&&this.id!=-1||this.playerData.name=='Buster'&&game.level==23)&&target.playerData.name!='PlayerSpy'&&target.fade>0&&!(this.playerData.name=='Buster'&&target.index!=this.target.index)
    }
    med(){
    ...
    doubleJump(){
        return (
            ...
            this.playerData.name=='PlayerQuarry'||this.playerData.name=='PlayerHeister'||this.playerData.name=='PlayerSpreadPistol'||
    ...
    update(){
        ...
        if((this.id==0||this.id>game.gaming)&&this.blindTime>0){
        ...
        }else if(((game.level==27||game.level==38)&&game.pvp?this.index+1:this.id)>game.gaming||this.id==0||this.auto||game.noPlayer){
            if(floor(random(0,30))==0||game.level==33&&abs(this.position.x-936)<100||abs(this.position.x-this.target.position.x)<10){
                if(game.attacker&&this.id==0&&!this.free&&this.playerData.name!='Buster'){
                ...
                }else if(game.level==15||game.level==18||game.level==19||game.level==31||game.level==42){
                    let targets=[]
                    this.target.position.x=this.position.x
                    this.target.position.y=game.edge[1]*0.1
                    this.manage[1]=false
                    ...
                    if(targets.length>0){
                        ...
                        if(targets.length>0){
                            ...
                            if(game.level==42&&inBoxBox(this,{position:{x:game.sectors[0][0],y:game.sectors[0][1]},width:game.sectors[0][2]-this.width/2,height:game.sectors[0][3]-this.height/2})){
                    ...
                    if(game.level==42&&(abs(this.position.x-game.tileset[0]*115)<100||abs(this.position.x-game.tileset[0]*166)<100)){
                        this.manage[2]=1
                    }
                    ...
                    if(
                        !game.noPlayer&&(
                            this.target.point==-1||
                            floor(random(0,(this.life<=0?15:150)))==0||
                    ...
                    if(targets.length>0){
                        ...
                        if(this.parachute){
                            ...
                            }else if(this.id>0&&(this.stats.points>=10||this.stats.points>=5&&floor(random(0,1800))==0)&&this.target.point>=1&&this.target.point<=4){
            ...
            if(this.disable){
                ...
                let jumpMult=(game.level==33?0.5:game.level==26&&this.wet>0?0.25:game.level==1||game.level==6?0.5:game.level==15||game.level==18?2:1)*(this.id>0?0.8:1)
                ...
                if(this.manage[0]==inputSwap&&this.life>0&&this.stunTime<=0&&this.stuckTime<=0&&!(game.attacker&&this.id==0&&!this.free&&this.playerData.name!='Buster'&&this.position.x<this.base.position.x-150)){
                    ...
                    if(!this.thrown&&!this.thrown2){
                        this.velocity.x-=(this.weaponType==-1?1.6:effectiveWeaponSpeed)*(game.level==6&&effectivePlayerSpeed<1?effectivePlayerSpeed*0.8+0.2:effectivePlayerSpeed)*(this.id>0&&game.randomizer?2:1)*(this.speedBuff>0?1.5:1)*(this.auto&&game.level==38?0.6:1)
                    }
                    this.runAnim(1/30)
            ...
            if(this.disable){
                ...
                let inputSet=inputs.keys[game.gaming==1?1:(game.level==27||game.level==38)&&game.pvp?this.index:this.id-1]
                let inputSetB=inputs.tap[game.gaming==1?1:(game.level==27||game.level==38)&&game.pvp?this.index:this.id-1]
                let inputSetC=inputs.release[game.gaming==1?1:(game.level==27||game.level==38)&&game.pvp?this.index:this.id-1]
                ...
                if(inputSet[inputSwap]&&!inputSet[1-inputSwap]&&this.life>0&&this.stunTime<=0&&this.stuckTime<=0){
                    this.direction.goal=-54
                    if(!this.thrown&&!this.thrown2){
                        this.velocity.x-=(this.weaponType==-1?(this.remote?effectivePlayerSpeed:1.6):effectiveWeaponSpeed*effectivePlayerSpeed)*(this.id>0&&game.randomizer?5/3:1)*(this.id!=1&&game.assault?0.45:1)*(this.speedBuff>0?1.5:1)
                    }
                    this.runAnim(1/30)
                ...
                if(inputSet[3]&&this.life>0&&this.playerData.name!='PlayerSplitter'&&this.playerData.name!='PlayerSquad'&&this.playerData.name!='PlayerDivision'&&this.playerData.name!='PlayerPointer'&&this.playerData.name!='PlayerFleet'&&this.playerData.name!='PlayerPatrol'&&this.playerData.name!='SidekickBonker'&&this.playerData.name!='SidekickDisappointmentGuard'&&this.playerData.name!='SidekickBonkerGuard'&&this.playerData.name!='SidekickGuillotine'&&this.playerData.name!='SidekickBorder'&&this.playerData.name!='SidekickToxin'&&this.playerData.name!='SidekickIceberg'&&this.playerData.name!='SidekickBonkhive'){
                    ...
                    }else if(this.playerData.name=='PlayerSwapper'){
                        if(this.subWeaponA.cooldown<=0&&this.subWeaponA.ammo>0&&this.subWeaponAType>=0){
                            this.attack(1)
                        }
        ...
        if(this.weaponType>=0){
            ...
            if(this.weapon.cooldown>0){
                this.weapon.cooldown-=this.playerData.reloadBuff*(game.brutal&&this.variant==11?3:1)*(this.confuseTime>0||this.dizzyTime>0?1/3:1)*(this.fort&&(game.level==22||game.level==25||game.level==32||game.level==35||game.level==36||game.level==37||game.level==38)?0.25:this.fort&&(game.level==23||game.level==26||game.level==27||game.level==28||game.level==33||game.level==40)?0.5:1)*(!game.peakWeapon&&this.fort?0.5:1)*((!game.peakWeapon||game.classicWeapon&&this.id>0&&this.id<=game.gaming)&&(this.playerData.name.includes('Deployer'))?2:1)*(((game.level==27||game.level==38)&&game.pvp?this.index+1:this.id)>game.gaming&&!this.construct&&!this.auto&&!this.fort&&(!game.pvp||game.gaming==1||game.level==27)?2:1)
            }
            if(this.weapon.reload>0){
                this.weapon.reload-=this.playerData.reloadBuff*(game.brutal&&this.variant==11?3:1)*(this.confuseTime>0||this.dizzyTime>0?1/3:1)*(this.fort&&(game.level==22||game.level==25||game.level==32||game.level==35||game.level==36||game.level==37||game.level==38)?0.25:this.fort&&(game.level==23||game.level==26||game.level==27||game.level==28||game.level==33||game.level==40)?0.5:1)*(!game.peakWeapon&&this.fort?0.5:1)*((!game.peakWeapon||game.classicWeapon&&this.id>0&&this.id<=game.gaming)&&(this.playerData.name.includes('Deployer'))?2:1)*(((game.level==27||game.level==38)&&game.pvp?this.index+1:this.id)>game.gaming&&!this.construct&&!this.auto&&!this.fort&&(!game.pvp||game.gaming==1||game.level==27)?2:1)
            ...
            if(!this.construct&&!this.sidekick&&!this.auto&&game.level!=13&&game.level!=14&&(
                ...
                this.id>game.gaming&&game.level!=23&&game.level!=26&&game.level!=27&&game.level!=33&&game.level!=38||game.attacker&&this.id!=0||this.storeWeapon
            )){
                if(this.storeWeapon){
                    this.storeWeapon=false
                }
            ...
            for(let a=0,la=entities.players.length;a<la;a++){
                if(entities.players[a].index==this.die.killer&&entities.players[a].index!=this.index){
                    ...
                    if(game.bust&&game.level!=22&&game.level!=23&&game.level!=25&&game.level!=26&&game.level!=28&&game.level!=35&&game.level!=36){
                        let threshold=(game.pvp?[1600,1500,1400,1300,1200][game.players-1]:game.attacker?[3200,2800,2400,2000,1600][game.players-1]:[8000,7000,6000,5000,4000][game.players-1])*(game.peakWeapon?1.5:1)*(game.level==19||game.level==31||game.level==42?5:1)*(game.level==24||game.level==38?2:1)*(game.level==32||game.level==33?2.5:1)
        ...
        if(this.life<=0){
            ...
            if(!this.dead){
                ...
                if(this.fort){
                    this.id=(game.level==27||game.level==38)&&game.pvp?(this.id==-1?-1:3-this.id):game.level==22?0:-1
                    this.attacking=false
                    this.manage[1]=false
                    this.setColor()
                }
                for(let a=0,la=entities.players.length;a<la;a++){
                    if(entities.players[a].index==this.die.killer){
                        ...
                        if(this.fort&&!((game.level==27||game.level==38)&&game.pvp)){
                            ...
                            this.setColor()
                        }else if(game.level==38&&this.id==-1){
                        ...
                        for(let a=0,la=entities.walls.length;a<la;a++){
                            for(let b=0,lb=entities.walls[a].length;b<lb;b++){
                                if((entities.walls[a][b].type==31||entities.walls[a][b].type==33||entities.walls[a][b].type==36)&&dist(entities.walls[a][b].position.x,entities.walls[a][b].position.y,this.position.x,this.position.y)<200){
                                    ...
                                    }else if(game.level==19||game.level==23||game.level==26||game.level==27||game.level==29||game.level==31||game.level==32||game.level==33||game.level==35||game.level==37||game.level==38||game.level==40||game.level==42){
                                        game.point[entities.walls[a][b].pos]=this.id
                ...
                if(this.die.timer>(game.assault?60:300)&&game.classicRespawn&&!game.past||this.id>game.gaming&&this.die.timer>600&&!game.past&&!game.classicRespawn&&!game.pvp){
                    ...
                    }else if(game.level==25||game.level==40){
                        for(let a=0,la=entities.walls.length;a<la;a++){
                            for(let b=0,lb=entities.walls[a].length;b<lb;b++){
                ...
                case 'PlayerColdfront':
                    ...
                    for(let a=0,la=entities.players.length;a<la;a++){
                        if(((entities.players[a].id==0?1:0)!=(this.id==0?1:0)||game.pvp&&entities.players[a].id!=this.id)&&inBoxBox({position:{x:this.position.x+(lsin(this.direction.main)<0?-80:80),y:this.position.y+this.offset.position.y-10},width:20,height:100},entities.players[a])&&!entities.players[a].fort){
                            if(entities.players[a].chillTime==0){
                                entities.players[a].color.skin.head=[200,250,250]
                                entities.players[a].color.skin.body=[190,240,240]
                                entities.players[a].color.skin.legs=[175,225,225]
                                entities.players[a].color.skin.arms=[180,230,230]
                                entities.players[a].base.color.skin.head=[200,250,250]
                                entities.players[a].base.color.skin.body=[190,240,240]
                                entities.players[a].base.color.skin.legs=[175,225,225]
                                entities.players[a].base.color.skin.arms=[180,230,230]
                            }
                ...
                for(let a=0,la=entities.players.length;a<la;a++){
                    if(inBoxBox({position:{x:(this.position.x/2+this.previous.position.x/2),y:(this.position.y/2+this.previous.position.y/2)},width:this.width,height:this.height},entities.players[a])&&(entities.players[a].id!=this.id&&game.pvp||entities.players[a].id==0&&this.id!=0||entities.players[a].id!=0&&this.id==0||entities.players[a].id==-1)&&!entities.players[a].dead&&!this.dead){
                        ...
                        if((this.weaponType==253||this.weaponType==400)&&(!entities.players[a].fort||entities.players[a].auto)){
                            ...
                            entities.players[a].takeDamage((this.weaponType==415?400:this.weaponType==461||this.weaponType==533?300:this.playerData.name=='DeadlyTank'?200:this.weaponType==194||this.weaponType==242||this.weaponType==243||this.weaponType==245||this.weaponType==246||this.weaponType==247||this.weaponType==347||this.weaponType==356||this.weaponType==370||this.weaponType==385||this.weaponType==398||this.weaponType==421||this.weaponType==433||this.weaponType==495||this.weaponType==541||this.weaponType==543||this.weaponType==634||this.weaponType==671?120:80)*(crit?3:1)*(entities.players[a].fort&&!entities.players[a].auto?0.025:1))
        ...
        if(this.parachute){
            this.velocity.x*=game.pvp||game.level==19||game.level==29||game.level==31?(game.assault?0.8:0.99):0.5
            this.velocity.y*=2/3
        ...
        if(this.chillTime>0){
            this.chillTime--
            this.velocity.x*=this.id==0?0.75:0.8
        }
