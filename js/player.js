class player{
    ...
    display(layer,offsetX=0,offsetY=0){
        ...
        switch(this.weaponType){
            ...
            case 530: case 574: case 639: case 642:
    ...
    takeDamage(damage){
        ...
        if(
            ...
            this.life-=damage*(game.level==27&&(game.pvp?this.index+1:this.id)>game.gaming&&(!game.pvp||game.gaming==1)?0.8:1)*(this.vulnerableTime>0?3:1)*(this.defendBuff>0?0.5:1)*(
                ...
                ?0.25:
                this.playerData.name=='PlayerBonkerception'
                ?0.5:
    ...
    update(){
        ...
        if(this.construct&&!this.remote||this.fort&&!this.auto){
        ...
        }else if((game.level==27&&game.pvp?this.index+1:this.id)>game.gaming||this.id==0||this.auto||game.noPlayer){
                ...
                if(game.attacker&&this.id==0&&!this.free&&this.playerData.name!='Buster'){
                    ...
                    if(
                        !game.noPlayer&&(
                            ...
                            this.playerData.name!='Buster'&&this.target.point>=0&&(entities.players[game.players+[1,4,0,2,3,6,5,8,7][this.target.point]].id==this.id?1:0)==(this.weaponType>=0?1:0)||
                    ...
                    if(game.pvp&&this.position.x<220&&this.position.y>1880&&game.gate[0]&&this.id==0){
                        ...
                        }else if(game.pvp&&this.position.x>2160&&this.position.x<2520&&this.position.y>1200&&this.position.y<1305){
                            switch(goalPoint){
                            ...
                            for(let a=0,la=game.sectors.length;a<la;a++){
                                if(inBoxBox(this,{position:{x:game.sectors[a][0],y:game.sectors[a][1]*9/8},width:game.sectors[a][2]-this.width/2,height:game.sectors[a][3]*9/8-this.height/2})){
                                    ...
                                    switch(a){
                                        ...
                                        break
                                        case 3:
                                            if(game.level==23&&this.position.x>1840&&this.position.x<2040&&this.position.y<1850&&this.position.y>1625){
                                                this.target.position.x=1940
                                                this.target.position.y=0
                                            }else if(game.level==23&&this.position.y>1850){
                                                switch(goalPoint){
                                                    case 0: 
            ...
            if(this.disable){
                ...
                let jumpMult=(game.level==23&&this.wet>0?0.25:game.level==1||game.level==6?0.5:game.level==15||game.level==18?2:1)*(this.id>0?0.8:1)
                let jumpMult2=game.level==23&&this.wet>0?2:1
                if(this.jumper()){
                    ...
                    if(this.manage[2]==0&&(floor(random(0,360*jumpMult))==0||floor(random(0,60*jumpMult))==0&&this.position.y>this.target.position.y+25)){
                        this.manage[2]=1
                    }else if(this.manage[2]==1&&(floor(random(0,30*jumpMult2))==0||floor(random(0,15*jumpMult2))==0&&this.position.y<this.target.position.y+25)){
                        this.manage[2]=0
                    }
                }else{
                    if(this.manage[2]==0&&(floor(random(0,90*jumpMult))==0||floor(random(0,30*jumpMult))==0&&this.position.y>this.target.position.y+25)){
                        this.manage[2]=1
            ...
            if(this.disable){
                ...
                let inputSet=inputs.keys[game.gaming==1?1:game.level==27&&game.pvp?this.index:this.id-1]
                let inputSetB=inputs.tap[game.gaming==1?1:game.level==27&&game.pvp?this.index:this.id-1]
                let inputSetC=inputs.release[game.gaming==1?1:game.level==27&&game.pvp?this.index:this.id-1]
        ...
        if(this.weaponType>=0){
            ...
            if(this.weapon.cooldown>0){
                this.weapon.cooldown-=this.playerData.reloadBuff*(game.brutal&&this.variant==11?3:1)*(this.confuseTime>0||this.dizzyTime>0?1/3:1)*(this.fort&&(game.level==22||game.level==25)?0.25:this.fort&&(game.level==23||game.level==26||game.level==27)?0.5:1)*(!game.peakWeapon&&this.fort?0.5:1)*((!game.peakWeapon||game.classicWeapon&&this.id>0&&this.id<=game.gaming)&&(this.playerData.name.includes('Deployer'))?2:1)*((game.level==27&&game.pvp?this.index+1:this.id)>game.gaming&&!this.construct&&!this.auto&&!this.fort&&(!game.pvp||game.gaming==1||game.level==27)?2:1)
            }
            if(this.weapon.reload>0){
                this.weapon.reload-=this.playerData.reloadBuff*(game.brutal&&this.variant==11?3:1)*(this.confuseTime>0||this.dizzyTime>0?1/3:1)*(this.fort&&(game.level==22||game.level==25)?0.25:this.fort&&(game.level==23||game.level==26||game.level==27)?0.5:1)*(!game.peakWeapon&&this.fort?0.5:1)*((!game.peakWeapon||game.classicWeapon&&this.id>0&&this.id<=game.gaming)&&(this.playerData.name.includes('Deployer'))?2:1)*((game.level==27&&game.pvp?this.index+1:this.id)>game.gaming&&!this.construct&&!this.auto&&!this.fort&&(!game.pvp||game.gaming==1||game.level==27)?2:1)
            ...
            if(!this.construct&&!this.sidekick&&!this.auto&&game.level!=13&&game.level!=14&&(
                ...
                )&&(this.id>0&&this.id<=game.gaming||(game.level==23||game.level==26||game.level==27)&&this.id>0)&&!game.attacker||
                this.id>game.gaming&&game.level!=23&&game.level!=26&&game.level!=27||game.attacker&&this.id!=0||this.storeWeapon
            )){
                if(this.storeWeapon){
                    this.storeWeapon=false
                }
        ...
        if(this.record.life<this.life){
            ...
            for(let a=0,la=entities.players.length;a<la;a++){
                if(entities.players[a].index==this.die.killer&&entities.players[a].index!=this.index){
                    ...
                    if(entities.players[a].stats.bust>=(game.pvp?[1600,1400,1200,1000][game.players-1]:game.attacker?[3200,2800,2400,2000][game.players-1]:[8000,7000,6000,5000][game.players-1])*(game.peakWeapon?1.5:1)*(game.level==19||game.level==31?5:1)*(game.level==24?2:1)&&game.bust&&game.level!=22&&game.level!=23&&game.level!=25&&game.level!=26&&game.level!=28&&entities.players[a].id>0&&game.players>1&&!entities.players[a].fort){
                    ...
                    if(entities.players[a].builder!=-1){
                        for(let b=0,lb=entities.players.length;b<lb;b++){
                            if(entities.players[b].index==entities.players[a].builder){
                                ...
                                if(entities.players[b].stats.bust>=(game.pvp?[1600,1400,1200,1000,900,800][game.players-1]:game.attacker?[3200,2800,2400,2000,1800,1600][game.players-1]:[8000,7000,6000,5000,4500,400][game.players-1])*(game.peakWeapon?1.5:1)*(game.level==19||game.level==31?5:1)*(game.level==24?2:1)&&game.bust&&game.level!=22&&game.level!=23&&game.level!=25&&game.level!=26&&game.level!=28&&entities.players[b].id>0&&game.players>1&&!entities.players[b].fort){
        ...
        if(this.life<=0){
            ...
            if(!this.dead){
                ...
                if(this.fort){
                    this.id=game.level==27&&game.pvp?3-this.id:game.level==22?0:-1
                    this.attacking=false
                    this.manage[1]=false
                    this.setColor()
                }
