class player{
    ...
    initialWeapon(){
        ...
        switch(this.playerData.name){
            ...
            break
            case 'PlayerSelector':
                let tick=game.peakWeapon?1:(game.level==27||game.level==38||game.level==44)&&game.pvp?0:floor(random(0,1.5))
            ...
            if(floor(random(0,30))==0||game.level==33&&abs(this.position.x-936)<100||abs(this.position.x-this.target.position.x)<10){
                if(game.attacker&&this.id==0&&!this.free&&this.playerData.name!='Buster'){
                    ...
                    if(this.playerData.name.includes('Deployer')){
                        ...
                        this.manage[1]=true
                    }else if(game.level==39&&(this.position.x<game.edge[0]*0.3||this.position.x>game.edge[0]*0.7)){
                        ...
                        for(let a=0,la=entities.walls.length;a<la;a++){
                            for(let b=0,lb=entities.walls[a].length;b<lb;b++){
                                if((entities.walls[a][b].type==31||entities.walls[a][b].type==33||entities.walls[a][b].type==36)&&dist(entities.walls[a][b].position.x,entities.walls[a][b].position.y,this.position.x,this.position.y)<200){
                                    ...
                                    }else if(game.level==19||game.level==23||game.level==26||game.level==27||game.level==29||game.level==31||game.level==32||game.level==33||game.level==35||game.level==37||game.level==38||game.level==40||game.level==42||game.level==43||game.level==44){
                                        game.point[entities.walls[a][b].pos]=this.id
                ...
                if(this.die.timer>(game.assault?60:300)&&game.classicRespawn&&!game.past||this.id>game.gaming&&this.die.timer>600&&!game.past&&!game.classicRespawn&&!game.pvp){
                    if(game.level==19||game.level==34){
                        ...
                        */
                        ...
                        if((game.point[0]==this.id||game.point[4]==this.id)&&this.die.timer>(this.id==1&&game.point[3]==1||this.id==2&&game.point[1]==2?360:this.id==1&&game.point[1]==1||this.id==2&&game.point[3]==2?480:720)){
