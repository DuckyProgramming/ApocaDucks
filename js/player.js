class player{
            ...
            if(game.newStats){
                if(this.playerData.name=='PlayerSpy'){
                    ...
                    if(game.level==30||game.level==54&&game.pvp){
                        ...
                        layer.text(`Damage: ${regNum(entities.players[this.copy].stats.damage)}\nDeaths: ${entities.players[this.copy].stats.deaths}\nWeapon: ${entities.players[this.copy].weaponType==-1?`None`:(game.classWeapon?cutName(entities.players[this.copy].subWeaponAData.name):entities.players[this.copy].weaponData.name)}`,0,-35)
                    }
    ...
    display(layer,offsetX=0,offsetY=0){
        ...
        if(this.parachute||this.rules.parachute||this.playerData.name=='PlayerDirigible'||this.playerData.name=='PlayerSoldier4'&&this.subPlayerAData.name=='PlayerLightParachutist'||this.playerData.name=='PlayerSoldierW'&&this.subPlayerAData.name=='PlayerLightParachutist'){
            layer.noFill()
        ...
        switch(this.playerData.name){
            ...
            case 'PlayerRevolutionistception':
                ...
                layer.ellipse(-35*lcos(-this.time),35*lsin(-this.time)-10,2.8)
            break
            case 'PlayerPlanetoid': case 'Planetoid':
                layer.stroke(150,this.fade)
                layer.strokeWeight(2)
                layer.noFill()
                layer.ellipse(0,-10,100)
    ...
    swapSubWeapons(){
        if(
            ...
            this.playerData.name=='PlayerEngineerW'&&this.effectiveId()<=game.gaming&&(this.subPlayerAData.name=='PlayerDeployerLevel'||this.subPlayerBData.name=='PlayerDeployerLevel'||this.subPlayerCData.name=='PlayerDeployerLevel')
        ){
    ...
    takeDamage(damage,spec=0){
        ...
        if(
            ...
            let charMult=
                ...
                ?2/3:
                this.rules.take45||this.playerData.name=='PlayerDemomanW'&&(this.subWeaponAType==982||this.subWeaponBType==982)&&spec==0
                ?0.8:
            ...
            if(this.id==0&&game.ender&&this.life>0&&!this.fort&&floor(random(0,5))==0||this.playerData.name.includes('Ender')){
                ...
                switch(this.playerData.name){
                    case 'EnderShotgunMartyr':
                        ...
                        entities.projectiles.push(new projectile(this.layer,this.position.x,this.position.y,30,-90,this.id,100,180,crit,this.index))
                        entities.projectiles.push(new projectile(this.layer,this.position.x,this.position.y,30,90,this.id,100,180,crit,this.index))
                    break
                }
            ...
            if(!((this.rules.spyLine||this.playerData.name=='PlayerSpyW'&&this.subWeaponCType!=1008&&this.subWeaponCType!=1021)&&this.fade<1&&(weaponType==684||weaponType==749||weaponType==885||weaponType==939||weaponType==940||weaponType==990||weaponType==991||weaponType==1019))&&!((this.playerData.name=='PlayerSpyC2'||this.playerData.name=='PlayerSpyW'&&this.subWeaponCType==1006)&&this.visible>=480)&&!((weaponType==725||weaponType==927)&&this.assort.firingTick<1)&&!(weaponType==928&&this.assort.firingTick<4/9)){
                ...
                switch(weaponType){
                    ...
                    case 619:
                        ...
                        this.inspect.push(entities.players[entities.players.length-1].index)
                    break
                    case 620: case 689: case 799: case 826: case 886: case 1010:
                        ...
                        if(!fired620[0]&&lsin(this.direction.main)<0){
                            for(let a=0,la=4;a<la;a++){
                                if(!((weaponType==689||weaponType==799||weaponType==826||weaponType==886||weaponType==1010)&&(a==0||a==3))){
                                    entities.projectiles.push(new projectile(this.layer,spawn[0],spawn[1]-6+a*4,weaponType==799?434:1,-90+3-a*2,this.id,weaponData.damage*damageBuff,300,crit,this.index))
                                    entities.projectiles[entities.projectiles.length-1].speed=8
                                }
                            }
                        }
                    ...
                    break
                    case 817:
                        entities.projectiles.push(new projectile(this.layer,spawn[0],spawn[1],363,(lsin(this.direction.main)<0?-90:90),this.id,weaponData.damage*damageBuff*1.1,300,crit,this.index))
                        if(!entities.projectiles[entities.projectiles.length-1].remove){
                            entities.projectiles.splice(entities.projectiles.length-1,1)
                            entities.projectiles.push(new projectile(this.layer,spawn[0],spawn[1],363,(lsin(this.direction.main)<0?-90:90)-30,this.id,weaponData.damage*damageBuff*1.1,300,crit,this.index))
                            if(!entities.projectiles[entities.projectiles.length-1].remove){
                                entities.projectiles.splice(entities.projectiles.length-1,1)
                                entities.projectiles.push(new projectile(this.layer,spawn[0],spawn[1],363,(lsin(this.direction.main)<0?-90:90)+30,this.id,weaponData.damage*damageBuff*1.1,300,crit,this.index))
                                if(!entities.projectiles[entities.projectiles.length-1].remove){
                                    entities.projectiles.splice(entities.projectiles.length-1,1)
                                    entities.projectiles.push(new projectile(this.layer,spawn[0],spawn[1],363,(lsin(this.direction.main)<0?-90:90)+15,this.id,weaponData.damage*damageBuff*1.1,300,crit,this.index))
                                    if(!entities.projectiles[entities.projectiles.length-1].remove){
                                        entities.projectiles.splice(entities.projectiles.length-1,1)
                                        entities.projectiles.push(new projectile(this.layer,spawn[0],spawn[1],363,(lsin(this.direction.main)<0?-90:90)+15,this.id,weaponData.damage*damageBuff*1.1,300,crit,this.index))
                                        if(!entities.projectiles[entities.projectiles.length-1].remove){
                    ...
                    case 883:
                        ...
                        if(round(weapon.uses)%4==0){
                            ...
                            entities.projectiles.push(new projectile(this.layer,spawn[0],spawn[1],219,(lsin(this.direction.main)<0?-90:90)+random(-2.5,2.5)*max(6-this.assort.firingTime/6,1),this.id,weaponData.damage*damageBuff*0.5,300,crit,this.index))
                        }
                    break
                    case 884:
                        for(let a=0,la=entities.players.length;a<la;a++){
                    ...
                    break
                    case 940:
                        entities.projectiles.push(new projectile(this.layer,spawn[0],spawn[1],403,(lsin(this.direction.main)<0?-90:90)+random(-1,1),this.id,weaponData.damage*damageBuff,10,crit,this.index))
                    break
                    case 941:
                        entities.projectiles.push(new projectile(this.layer,spawn[0],spawn[1],1,(lsin(this.direction.main)<0?-90:90)+random(-1.5,1.5),this.id,weaponData.damage*damageBuff,300,crit,this.index))
                    ...
                    break
                    case 990:
                        entities.projectiles.push(new projectile(this.layer,spawn[0],spawn[1],419,(lsin(this.direction.main)<0?-90:90)+random(-1,1),this.id,weaponData.damage*damageBuff,10,crit,this.index))
                    break
                    case 991:
                        entities.projectiles.push(new projectile(this.layer,spawn[0],spawn[1],420,(lsin(this.direction.main)<0?-90:90)+random(-1,1),this.id,weaponData.damage*damageBuff,10,crit,this.index))
                    break
                    case 993:
                        entities.projectiles.push(new projectile(this.layer,spawn[0],spawn[1],1,(lsin(this.direction.main)<0?-90:90)+random(-1.5,1.5),this.id,weaponData.damage*damageBuff,300,crit,this.index))
                    ...
                    break
                    case 1019:
                        entities.projectiles.push(new projectile(this.layer,spawn[0],spawn[1],432,(lsin(this.direction.main)<0?-90:90)+random(-1,1),this.id,weaponData.damage*damageBuff,10,crit,this.index))
                    break
                    case 1020:
    ...
    update(){
        ...
        if((this.id==0||this.effectiveId()>game.gaming)&&this.blindTime>0){
                ...
                for(let a=0,la=entities.players.length;a<la;a++){
                    if(
                        ...
                        if(this.fort&&abs(this.position.x-entities.players[a].position.x)<(this.weaponType==413?240:this.weaponType==58?720:600)&&abs(this.position.y-entities.players[a].position.y)<45||!this.fort&&(
                            abs(this.position.x-entities.players[a].position.x)<600&&abs(this.position.y-entities.players[a].position.y)<abs(this.position.x-entities.players[a].position.x)/10+40&&this.weaponType!=6&&this.weaponType!=8&&this.weaponType!=11&&this.weaponType!=99&&this.weaponType!=413&&this.weaponType!=689&&this.weaponType!=730&&this.weaponType!=731&&this.weaponType!=732&&this.weaponType!=799&&this.weaponType!=826&&this.weaponType!=886&&this.weaponType!=944&&this.weaponType!=1010&&this.weaponType!=1018||
                            ...
                            abs(this.position.x-entities.players[a].position.x)<750&&abs(this.position.y-entities.players[a].position.y)<abs(this.position.x-entities.players[a].position.x)/3+40&&(this.weaponType==689||this.weaponType==730||this.weaponType==731||this.weaponType==732||this.weaponType==799||this.weaponType==886||this.weaponType==1010||this.weaponType==1018)||
                            abs(this.position.x-entities.players[a].position.x)<450&&abs(this.position.y-entities.players[a].position.y)<abs(this.position.x-entities.players[a].position.x)/3+40&&this.weaponType==826||
                            abs(this.position.x-entities.players[a].position.x)<375&&abs(this.position.y-entities.players[a].position.y)<abs(this.position.x-entities.players[a].position.x)/3+40&&this.weaponType==944
                        )&&entities.players[a].life>0&&
            ...
            if(this.disable){
                ...
                if(!this.decoy&&this.manage[2]==1&&this.life>0&&this.stunTime<=0&&(this.jump.time>3||this.jump.double==1&&this.velocity.y>0||this.jump.active>0)&&this.stuckTime<=0&&!(game.attacker&&this.id==0&&!this.free&&!this.playerData.name.includes('Buster')&&this.position.y<this.base.position.y+50)&&this.stunTime<=0){
                    if(this.jump.time>3){
                        this.jump.time=0
                    ...
                    this.jumpAction()
                }else if(this.playerData.name==`ShotgunVault`&&this.life<=this.base.life*0.5&&this.jump.time>3&&this.stuckTime<=0&&!this.assort.vault){
                ...
                if(
                    ...
                    if(this.manage[1]==1||this.rules.class&&this.subWeaponAType==879&&this.subWeaponA.ammo<this.subWeaponAData.ammo){
                        ...
                        if(this.playerData.name=='PlayerConglomeration'){
                            ...
                            if((this.rules.scoutLine&&this.subPlayerAData.name==`PlayerPistolW`/*this.subWeaponAType!=879*/||this.playerData.name=='PlayerEngineerC7'||this.rules.classW&&(this.subPlayerAData.name==`PlayerPistolW`||this.subPlayerAData.name==`PlayerPistolC`||this.subPlayerAData.name==`PlayerPistol`))&&this.subWeaponA.uses>0&&this.assort.firing<20){
                                this.subWeaponA.cooldown=0
                            }
                            if(this.subWeaponA.cooldown<=0&&this.subWeaponA.ammo>0&&this.subWeaponAType>=0&&!this.subWeaponA.reloading){
                                if(this.playerData.name=='PlayerSniperW'&&this.subWeaponAType==728){
                        ...
                        if(this.subPlayerAData.name!='PlayerDirector'&&this.subPlayerAData.name!='PlayerSwarmer'&&this.subPlayerAData.name!='PlayerMotorizer'&&this.subPlayerAData.name!='PlayerHeavyDirector'&&this.subPlayerAData.name!='PlayerHeavySwarmer'&&this.subPlayerAData.name!='PlayerLightSkysweeper'&&this.subPlayerAData.name!='PlayerHeavyMotorizer'&&this.subPlayerAData.name!='PlayerOrbital'&&floor(random(0,15))==0){
                            this.swapSubWeapons()
                        }
                        if(this.subWeaponA.cooldown<=0&&this.subWeaponA.ammo>0&&this.subWeaponAType>=0&&!this.subWeaponA.reloading){
                            this.attack(1)
            ...
            if(this.disable){
                ...
                if(inputSet[2]&&this.life>0&&this.stunTime<=0&&(this.jump.time>3||this.jump.active>0||this.jump.double==1&&inputSetB[2]||this.jump.triple==1&&inputSetB[2]||this.jump.quadruple==1&&inputSetB[2])&&this.stuckTime<=0){
                    if(this.jump.time>3){
                        this.jump.time=0
                        this.jump.active=ceil((10-floor(max(0,this.assort.tired-this.life/this.base.life*30)/30))*(this.chillTime>0?0.5:1))
                        this.assort.tired+=30
                    ...
                    this.jumpAction()
                }else if(this.rules.qBonusJump&&inputSetC[0]&&this.life>0&&this.jump.time>3&&this.stuckTime<=0){
                    this.jump.time=0
                    this.velocity.x=lsin(this.direction.main)*30
                    this.velocity.y=-25
                    this.thrown=true
                    ...
                    if((
                        ...
                        if(this.weaponType>=0){
                            if(this.playerData.name=='PlayerConglomeration'){
                                ...
                                if((this.rules.scoutLine&&this.subPlayerAData.name==`PlayerPistolW`/*this.subWeaponAType!=879*/||this.playerData.name=='PlayerEngineerC7'||this.rules.classW&&(this.subPlayerAData.name==`PlayerPistolW`||this.subPlayerAData.name==`PlayerPistolC`||this.subPlayerAData.name==`PlayerPistol`))&&this.subWeaponA.uses>0&&inputSetB[3]/*&&this.subWeaponA.cooldown<20*/){
                                    this.subWeaponA.cooldown=0
                                }
                                if(this.playerData.name=='PlayerSniperW'&&this.subWeaponAType==728){
                                    if(this.subWeaponB.cooldown<=0&&this.subWeaponB.ammo>0&&this.subWeaponBType>=0){
            ...
            if(this.selector>=this.inputs.length){
                ...
                if(this.inputs[this.selector][2]&&this.life>0&&this.stunTime<=0&&(this.jump.time>3||this.jump.active>0)&&this.stuckTime<=0){
                    if(this.jump.time>3){
                        this.jump.time=0
                        this.jump.active=ceil((10-floor(max(0,this.assort.tired-this.life/this.base.life*30)/30))*(this.chillTime>0?0.5:1))
                        this.assort.tired+=30
            ...
            if(!this.dead){
                ...
                switch(this.playerData.name){
                    ...
                    case 'PistolSplitterSplitter':
                        for(let a=0,la=2;a<la;a++){
                            entities.players.push(new player(this.layer,this.position.x,this.position.y+this.height/2-15,0,0,[],true,findName('PistolSplitter',types.player),game.index))
                            game.index++
                            entities.players[entities.players.length-1].free=true
                            entities.players[entities.players.length-1].lastingForce[0]+=[0.25,-0.25][a]
                            entities.players[entities.players.length-1].weapon.cooldown+=60
                    ...
                    case 'PistolSplitterSplitterSplitter':
                        for(let a=0,la=2;a<la;a++){
                            entities.players.push(new player(this.layer,this.position.x,this.position.y+this.height/2-18,0,0,[],true,findName('PistolSplitterSplitter',types.player),game.index))
                            game.index++
                            entities.players[entities.players.length-1].free=true
                            entities.players[entities.players.length-1].lastingForce[0]+=[0.25,-0.25][a]
                            entities.players[entities.players.length-1].weapon.cooldown+=60
                    ...
                    case 'HyperPistolSplitterSplitter':
                        for(let a=0,la=2;a<la;a++){
                            entities.players.push(new player(this.layer,this.position.x,this.position.y+this.height/2-15,0,0,[],true,findName('HyperPistolSplitter',types.player),game.index))
                            game.index++
                            entities.players[entities.players.length-1].free=true
                            entities.players[entities.players.length-1].lastingForce[0]+=[0.25,-0.25][a]
                            entities.players[entities.players.length-1].weapon.cooldown+=60
                    ...
                    case 'HyperMedicSplitterSplitter':
                        for(let a=0,la=2;a<la;a++){
                            entities.players.push(new player(this.layer,this.position.x,this.position.y+this.height/2-15,0,0,[],true,findName('HyperMedicSplitter',types.player),game.index))
                            game.index++
                            entities.players[entities.players.length-1].free=true
                            entities.players[entities.players.length-1].lastingForce[0]+=[0.25,-0.25][a]
                            entities.players[entities.players.length-1].weapon.cooldown+=60
                    ...
                    break
                    case 'Mystery':
                        entities.players.push(new player(this.layer,this.position.x,this.position.y+this.height/2,this.id,0,[],false,floor(random(findName('Wait',types.player)+1,types.player.length)),game.index))
                        game.index++
                        entities.players[entities.players.length-1].free=true
                        entities.players[entities.players.length-1].weapon.cooldown+=60
        ...
        if(this.life>0){
            ...
            switch(this.playerData.name){
                case 'RocketLauncherHeal': case 'BigRocketLauncherHeal': case 'FastPunchHeal': case 'PistolHeal': case 'CritRocketLauncherHeal': case 'SpyHeal': case 'HeavyPunchHeal': case 'EngineerHeal': case 'BigCritPistolHeal': case 'FlamethrowerHeal': case 'MachineGunHeal':
                    for(let a=0,la=entities.players.length;a<la;a++){
                        if(dist(this.position.x,this.position.y,entities.players[a].position.x,entities.players[a].position.y)<240&&this.position.x!=entities.players[a].position.x&&!entities.players[a].dead&&!this.dead&&this.id==entities.players[a].id&&!entities.players[a].fort&&!entities.players[a].playerData.name.includes('Tank')){
                ...
                case 'BigMedicAura': case 'MedicAura': case 'HyperMedicAura': case 'PlusMedicAura':
                    for(let a=0,la=entities.players.length;a<la;a++){
                        if(dist(this.position.x,this.position.y,entities.players[a].position.x,entities.players[a].position.y)<360&&this.position.x!=entities.players[a].position.x&&!entities.players[a].dead&&!this.dead&&(this.id==0?1:0)==(entities.players[a].id==0?1:0)&&!entities.players[a].playerData.name.includes('Tank')){
                            entities.players[a].life=min(max(entities.players[a].base.life,entities.players[a].life),entities.players[a].life+sqrt(entities.players[a].base.life)/120)
                        }
                    }
                break
                ...
                case 'PlayerPlanetoid': case 'Planetoid':
                    for(let a=0,la=entities.players.length;a<la;a++){
                        for(let b=0,lb=4;b<lb;b++){
                            if(inBoxBox({position:{x:this.position.x+50*lsin(this.time+b*90),y:this.position.y+50*lcos(this.time+b*90)-10},width:16,height:16},entities.players[a])&&this.validTarget(entities.players[a])&&!entities.players[a].dead&&!this.dead){
        ...
        this.assort.tired=this.wet>0||this.id>game.gaming&&!game.pvp?0:constrain(this.assort.tired-(this.assort.tiredTick>0?0.4:3),0,150)
        if(this.wet>0){
            this.wet--
        }
        ...
        if(this.id==0&&!game.body||this.construct||this.sidekick||this.remote||this.auto||this.decoy){
            ...
            if(this.invincible>0){
                ...
                if(this.life<=0){
                    ...
                    this.fade=smoothAnim(this.fade,this.assort.intel||(this.visible==0||this.visible2>0)&&!(this.playerData.name=='PlayerSpyW'&&this.speedBuff>300&&(this.subWeaponAType==749||this.subWeaponBType==749||this.subWeaponAType==970||this.subWeaponBType==970)),0,this.visible==0||this.assort.intel?1:0.4,this.visible>595?5:this.playerData.name=='PlayerSpyC2'||this.playerData.name=='PlayerSpyW'&&(this.speedBuff<=0||this.subWeaponAType!=749&&this.subWeaponBType!=749&&this.subWeaponAType!=970&&this.subWeaponBType!=970)?48:12)
                }else{
