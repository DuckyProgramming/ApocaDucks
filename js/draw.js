    ...
    switch(stage.scene){
                ...
                for(let b=0,lb=[5,5,2,5,5,5,5,5,1][a];b<lb;b++){
                    ...
                    text(
                        [
                            ...
                            ['Alloy','Speleo','NuclearMountain','Razorpoint','Entropy'][b],
                            ['Blueprint'][b],
                        ][a],pos[0],pos[1]
                    )
                    textSize(11.25)
            ...
            for(let a=0,la=types.mission[game.mission].wave.length;a<la;a++){
                ...
                if(lb>44){
                    ...
                    for(let b=0;b<lb;b++){
                        if(types.mission[game.mission].wave[a][b][0]!='Wait'){
                            let num=types.mission[game.mission].wave[a][b][1]
                            text(`${types.mission[game.mission].wave[a][b][0].includes('Boss')?num:ceil(num*(menu.level==37?game.players*0.1+0.25:menu.level==29?game.players*0.1+0.4:game.players*0.25+0.25)*(game.classicRespawn?2:1)*(menu.level==7?0.6:1)*(menu.level==8?(game.attacker?0.75:1.5):1)*(menu.level==16?0.5:1)*(menu.level==17?(game.attacker?0.4:1):1)*(menu.level==19||menu.level==31||menu.level==42?2.5:1)*(menu.level==29||menu.level==37?(types.mission[game.mission].wave.length==1?0.3:1.8):1)*(menu.level==32||menu.level==33?1.2*(menu.level==42?0.75:1):1)*(menu.weapon==1?2:1)*game.diff)} x ${types.mission[game.mission].wave[a][b][0]}`,width/2+(a%4)*210-315,height/2-265+ticker*14.375+floor(a/4)*310)
                            ticker++
                        }
                    }
                }
        ...
        case 'main':
            ...
            for(let c=0,lc=game.gaming;c<lc;c++){
                if(game.level==15||game.level==18||game.level==30||game.level==36||game.level==37||game.level==38||game.level==40||game.level==41||game.level==43||game.level==44){
                ...
                if(!special){
                    key[c]*=dev.sight?11:entities.players[c].blindTime>0?0.5:entities.players[c].parachute?(game.level==39||game.level==41||game.level==43?2:3):
                        ...
                        entities.players[c].weaponType==673||
