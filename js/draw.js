    ...
    switch(stage.scene){
                ...
                for(let b=0,lb=[5,5,2,5,5,5,5][a];b<lb;b++){
                    ...
                    text(
                        [
                            ...
                            ['Arizona','Aerial','Valuation','Tiny Rock (WIP)','Blueprint'][b]
                        ][a],pos[0],pos[1]
                    )
                    textSize(11.25)
                    switch(a){
                ...
                if(lb>44){
                    ...
                    for(let b=0;b<lb;b++){
                        if(types.mission[game.mission].wave[a][b][0]!='Wait'){
                            ...
                            text(`${types.mission[game.mission].wave[a][b][0].includes('Boss')?num:ceil(num*(menu.level==37?game.players*0.1+0.2:menu.level==29?game.players*0.1+0.4:game.players*0.25+0.25)*(game.classicRespawn?2:1)*(menu.level==7?0.6:1)*(menu.level==8?(game.attacker?0.75:1.5):1)*(menu.level==16?0.4:1)*(menu.level==17?(game.attacker?0.4:1):1)*(menu.level==19?2.5:1)*(menu.level==29||menu.level==37?(types.mission[game.mission].wave.length==1?0.3:1.8):1)*(menu.level==32||menu.level==33?1.2:1)*(menu.weapon==1?1.25:1)*game.diff)} x ${types.mission[game.mission].wave[a][b][0]}`,width/2+(a%4)*210-315+ticker2*210,height/2-265+ticker*14.375+floor(a/4)*310)
                            ticker++
                            if(ticker>=40){
                                ticker-=40
                                ticker2++
        ...
        case 'main':
            ...
            for(let c=0,lc=game.gaming;c<lc;c++){
                ...
                if(!special){
                    key[c]*=dev.sight?8:entities.players[c].parachute?3:
