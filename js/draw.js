        ...
        case 'main':
            ...
            for(let c=0,lc=game.gaming;c<lc;c++){
                if(
                    ...
                    game.level==67||game.level==68||game.level==69||game.level==71||game.level==72||game.level==73||game.level==74||game.level==75
                ){
                ...
                if(!special){
                    key[c]*=dev.sight?game.edge[0]/graphics.main[0].width*(game.level==44?0.25:1):entities.players[c].blindTime>0?0.5:entities.players[c].parachute?(game.level==39||game.level==41||game.level==43||game.level==52?2:3):
                        ...
                        entities.players[c].weaponType==769||entities.players[c].weaponType==790||
            ...
            for(let a=0,la=game.gaming;a<la;a++){
                ...
                if(!game.attacker){
                    switch(game.level){
                        ...
                        case 68: case 69: case 70: case 76:
                            if(game.margin){
                                graphics.main[a].noFill()
                                graphics.main[a].stroke(255)
                                graphics.main[a].strokeWeight(10)
            ...
            for(let a=0,la=bs.length;a<la;a++){
                for(let b=0,lb=bs[a].length;b<lb;b++){
                    if(!(bs[a][b][0]==(game.level==54||game.level==55||game.level==65||game.level==67||game.level==68||game.level==69?3:2)&&(run.fore[bs[a][b][0]][bs[a][b][1]].type==31||run.fore[bs[a][b][0]][bs[a][b][1]].type==33||run.fore[bs[a][b][0]][bs[a][b][1]].type==36||run.fore[bs[a][b][0]][bs[a][b][1]].type==42))){
